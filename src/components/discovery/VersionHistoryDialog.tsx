      ...
      <DialogContent className="sm:max-w-[700px]">
        ...
        <Tabs defaultValue="versions" value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="versions" className="space-y-4">
            <div className="border rounded-md overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {versions.map((version, index) => <tr key={version.version} className={index !== versions.length - 1 ? "border-b" : ""}>
          ...
          <TabsContent value="changelog" className="space-y-6">
            {versions.map(version => <div key={version.version} className="border-b pb-4 last:border-0">
                <h3 className="text-lg font-semibold mb-2">
                  {version.version} - {formatDate(version.releaseDate)}
                </h3>
                
                {version.changes && version.changes.length > 0 ? <ul className="space-y-2 list-disc list-inside">
                    {version.changes.map((change, idx) => <li key={idx} className="text-sm text-muted-foreground">{change}</li>)}
                  </ul> : <p className="text-sm text-muted-foreground">No change log</p>}
              </div>)}
          </TabsContent>
        </Tabs>
      </DialogContent>
