...
export function ShareServerDialog({
  ...
  const versionHistory: VersionInfo[] = [{
    version: "0.9.5",
    releaseDate: new Date(2025, 3, 3),
    // April 3, 2025
    author: "API Team",
    changes: ["Added Kubernetes 1.28 support", "Improved cluster monitoring with real-time metrics", "Fixed pod visualization in dark mode"]
  }, {
    version: "0.9.0",
    releaseDate: new Date(2025, 2, 15),
    // March 15, 2025
    author: "API Team",
    changes: ["Launched deployment automation features", "Added support for custom namespaces", "Fixed several stability issues with long-running operations"]
  }, {
    version: "0.8.5",
    releaseDate: new Date(2025, 1, 20),
    // February 20, 2025
    author: "API Team",
    changes: ["Beta release of resource monitoring dashboard", "Initial implementation of pod visualization", "Added basic cluster management functionality"]
  }];
  ...
  const description = serverDefinition?.description || ('description' in server ? server.description : "A powerful server that enhances your development workflow");
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-xl md:max-w-2xl p-6">
          ...
          <div className="space-y-5 mt-2 border-t pt-4">
              ...
              <Select value={selectedConfig} onValueChange={setSelectedConfig}>
                <SelectTrigger id="config-select" className="w-full h-10">
                  <SelectValue placeholder="Select configuration">
                    {selectedConfig === "no-config" ? <div className="flex items-center gap-2">
                        <Ban className="h-4 w-4 text-muted-foreground" />
                        <span>No Config</span>
                      </div> : configOptions.find(option => option.value === selectedConfig)?.label || server.name}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  {configOptions.map(option => <SelectItem key={option.id} value={option.value}>
                      {option.id === "no-config" ? <div className="flex items-center gap-2">
                          <Ban className="h-4 w-4 text-muted-foreground" />
            ...
            {shareUrl ? <div className="space-y-3">
                <div>
                ...
                <div className="flex justify-end">
                  <Button variant="outline" size="sm" className={`gap-1.5 ${isCopied ? 'bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-900 dark:text-green-300 dark:hover:bg-green-800' : ''}`} onClick={handleCopyUrl} style={{
                ...
                <Button variant="default" size="sm" className="gap-1.5" onClick={generateShareUrl} disabled={isGeneratingLink}>
